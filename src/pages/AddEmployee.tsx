...
import { Form, Input, Select, DatePicker, InputNumber, Row, Col, Button, Breadcrumb } from 'antd';
...
export const AddEmployee: React.FC = () => {
  ...
  const onFinish = (values: any) => {
    ...
    const payload = {
      ...values,
      dob: values.dob ? values.dob.format('MM/DD/YYYY') : undefined,
      doj: values.doj ? values.doj.format('MM/DD/YYYY') : undefined,
      experience: `${values.experienceYears || 0} years ${values.experienceMonths || 0} months`,
    };
        ...
        <div style={{ backgroundColor: palette.surface, borderColor: palette.border }} className="rounded-xl p-6 border shadow-sm">
          ...
          >
            <Row gutter={[16, 16]}>
              ...
              <Col xs={24} md={12}>
                <Form.Item name="personalEmail" label="Personal Email" rules={[{ type: 'email', message: 'Enter a valid email',required: true }]}>
                  <Input />
                </Form.Item>
              </Col>

              <Col xs={24} md={12}>
                <Form.Item name="phone" label="Phone Number" rules={[{ pattern: /^\+?[0-9\- ]{7,15}$/, message: 'Enter a valid phone', }]}>
                  <Input />
                </Form.Item>
              </Col>

              <Col xs={24} md={12}>
                <Form.Item name="companyEmail" label="Company Email" rules={[{ type: 'email', message: 'Enter a valid company email' }]}>
                  <Input />
                </Form.Item>
              </Col>
                  ...
                  <Row gutter={8} align="middle">
                    <Col xs={12} md={12}>
                      <Form.Item name="experienceYears" noStyle>
                        <InputNumber min={0} max={60} placeholder="Years" style={{ width: '100%' }} />
                      </Form.Item>
                    </Col>
                    ...
                    <Col xs={12} md={12}>
                      ...
                      <Form.Item name="experienceMonths" noStyle>
                        <InputNumber min={0} max={11} placeholder="Months" style={{ width: '100%' }} />
                      </Form.Item>
                    </Col>
                  </Row>
                  <div style={{ color: palette.textSecondary, marginTop: 8 }}>Enter total prior experience at the time of joining (years and months).</div>
                </Form.Item>
              </Col>
            </Row>

            <div className="mt-6 flex justify-end gap-3">
              <Button type="default" onClick={() => navigate('/employees')} style={{ border: `1px solid ${palette.border}`, background: 'transparent', color: palette.textPrimary }}>
